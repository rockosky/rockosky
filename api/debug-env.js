...
//
// DELETE THIS FILE once SUPBASE_URL is confirmed working -- it's
// diagnostic-only, not meant to stay in production long-term.

module.exports = async (req, res) => {
  res.status(200).json({
    SUPBASE_URL_present: !!process.env.SUPBASE_URL,
    SUPBASE_URL_preview: (process.env.SUPBASE_URL || '(not set)').slice(0, 40),
    SUPBASE_SERVICE_ROLE_KEY_present: !!process.env.SUPBASE_SERVICE_ROLE_KEY,
