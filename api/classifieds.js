...
export default async function handler(req, res) {
  ...
  try {
    // Dynamic import (not a static top-of-file import) -- this is
    // deliberate. A static `import { createClient } from '...'` at
    // the top of the file runs BEFORE this try block even exists, so
    // if the package itself is missing/broken, it crashes with a
    // blank Vercel error page no try/catch can intercept. Using
    // `await import(...)` here means even a broken/missing package
    // gets caught below and reported as a real JSON error message --
    // no Vercel dashboard log-digging required to see what's wrong.
    let createClient;
    try {
      ({ createClient } = await import('@supabase/supabase-js'));
    } catch (importErr) {
      return res.status(500).json({ error: `Could not load @supabase/supabase-js: ${importErr.message}` });
    }
