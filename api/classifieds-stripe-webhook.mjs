  ...
  <div id="kf-cl-browse">
    <div class="kf-cl-top">
      ...
      <div class="kf-cl-account">
        ...
        <span id="kf-cl-token-balance" style="display:none;margin-left:8px;color:var(--kf-red);font-weight:700;cursor:pointer;"></span>
...
<!-- Post a listing -->
<div class="kf-cl-modal-backdrop" id="kf-cl-tokens-backdrop">
  <div class="kf-cl-modal" style="max-width:460px;">
    <button class="close-x" id="kf-cl-tokens-close">&times;</button>
    <h2>Buy White Donuts</h2>
    <div style="font-size:12.5px;color:var(--muted);margin-bottom:14px;">Preload credits and spend them instantly on any listing — no card entry at checkout.</div>
    <div id="kf-cl-tokens-list">Loading…</div>
    <div id="kf-cl-tokens-checkout-form" style="margin-top:14px;"></div>
    <div class="kf-cl-modal-msg" id="kf-cl-tokens-msg"></div>
  </div>
</div>
...
(function () {
  ...
  async function updateAccountUI(user) {
    ...
    const tokenEl = document.getElementById('kf-cl-token-balance');
    if (user) {
      ...
      refreshTokenBalance(user.id);
      tokenEl.style.display = 'inline';
      tokenEl.onclick = openTokensModal;
    } else {
  ...
  async function refreshTokenBalance(userId) {
    try {
      const res = await fetch(`${API_BASE}/wallet-tokens?balance=1&userId=${userId}`);
      const data = await res.json();
      currentTokenBalance = data.balance_tokens || 0;
      document.getElementById('kf-cl-token-balance').textContent = `🍩 ${currentTokenBalance.toLocaleString()} White Donuts`;
    } catch (err) { /* leave as-is on error */ }
  }
  ...
  window.kfClBuyWithTokens = async function (listingId, priceCents) {
    const btn = document.getElementById('kf-cl-buy-tokens-btn');
    ...
    try {
      const res = await fetch(`${API_BASE}/classifieds-checkout`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ listingId, buyerId: currentUser.id, buyerEmail: currentUser.email, payWithTokens: true })
      });
      const data = await res.json();

      if (res.ok && data.paidWithTokens) {
        currentTokenBalance = data.new_balance;
        document.getElementById('kf-cl-token-balance').textContent = `🍩 ${currentTokenBalance.toLocaleString()} White Donuts`;
        msgEl.textContent = 'Purchased with White Donuts! 🎉';
        msgEl.className = 'kf-cl-modal-msg ok';
        btn.textContent = 'Purchased ✓';
        setTimeout(() => kfClOpenDetail(listingId), 1200);
      } else {
        msgEl.textContent = data.error || 'Could not complete purchase.';
        msgEl.className = 'kf-cl-modal-msg err';
        btn.disabled = false;
        btn.textContent = '🍩 Pay with White Donuts';
      }
    } catch (err) {
  ...
  // ---- Buy White Donuts: lists purchasable token packages, and runs
  // an embedded Stripe form for whichever one is picked -- same
  // client-side pattern as the listing checkout below, just a
  // different backend endpoint and a wallet credit instead of a sale. ----
  const tokensBackdrop = document.getElementById('kf-cl-tokens-backdrop');

  async function openTokensModal() {
    if (!currentUser) { authBackdrop.classList.add('open'); return; }
    tokensBackdrop.classList.add('open');
    document.getElementById('kf-cl-tokens-checkout-form').innerHTML = '';
    document.getElementById('kf-cl-tokens-msg').textContent = '';
    const listEl = document.getElementById('kf-cl-tokens-list');
    listEl.innerHTML = 'Loading…';

    try {
      const res = await fetch(`${API_BASE}/wallet-tokens?packages=1&userId=${currentUser.id}`);
      const { packages } = await res.json();
      if (!packages || !packages.length) { listEl.innerHTML = '<div class="empty">No bundles available right now.</div>'; return; }

      listEl.innerHTML = packages.map(p => `
        <div style="display:flex;justify-content:space-between;align-items:center;border:1px solid var(--border);border-radius:10px;padding:12px 14px;margin-bottom:10px;">
          <div>
            <div style="font-weight:700;">${p.name}</div>
            <div style="font-size:12px;color:var(--muted);">🍩 ${p.token_amount.toLocaleString()} White Donuts${p.is_first_purchase_only ? ' · first purchase only' : ''}</div>
          </div>
          <button class="submit ready" style="background:var(--kf-red);width:auto;padding:8px 16px;" onclick="kfBuyTokenPackage('${p.id}')">$${(p.price_cents / 100).toFixed(2)}</button>
        </div>
      `).join('');
    } catch (err) {
      listEl.innerHTML = '<div class="empty">Could not load bundles.</div>';
    }
  }

  document.getElementById('kf-cl-tokens-close').addEventListener('click', () => tokensBackdrop.classList.remove('open'));

  window.kfBuyTokenPackage = async function (packageId) {
    const msgEl = document.getElementById('kf-cl-tokens-msg');
    msgEl.textContent = 'Preparing checkout…'; msgEl.className = 'kf-cl-modal-msg';
    try {
      const res = await fetch(`${API_BASE}/wallet-tokens`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: currentUser.id, email: currentUser.email, packageId })
      });
      const data = await res.json();
      if (!res.ok || !data.client_secret) {
        msgEl.textContent = data.error || 'Could not start checkout.'; msgEl.className = 'kf-cl-modal-msg err';
        return;
      }
      msgEl.textContent = '';
      await mountEmbeddedForm(data.client_secret, 'kf-cl-tokens-checkout-form', async () => {
        tokensBackdrop.classList.remove('open');
        await refreshTokenBalance(currentUser.id);
      });
    } catch (err) {
      msgEl.textContent = 'Could not reach checkout — try again.'; msgEl.className = 'kf-cl-modal-msg err';
    }
  };

  // Shared embedded-form mounting logic, used by both the listing
  // checkout and the token purchase checkout.
  async function mountEmbeddedForm(clientSecret, containerId, onSuccess) {
    if (!stripeClient) return;
    const appearance = {
      theme: 'stripe', labels: 'auto', inputs: 'spaced',
      variables: {
        borderRadius: '8px', colorBackground: '#161311', colorDanger: '#e2231a',
        colorPrimary: '#e2231a', colorSuccess: '#5fbf7a', colorText: '#f2ece1',
        fontFamily: 'default', fontSizeBase: '14px', spacingUnit: '4px'
      }
    };
    ...
    if (loadActionsResult.type === 'success') {
      form.on('confirm', async (event) => {
        ...
        } catch (error) { /* surfaced by Stripe's own form UI */ }
      });
    }
  }
          ...
          <div>
            ...
            ${listing.status === 'active'
              ? `<button class="submit ready" id="kf-cl-buy-btn" style="background:var(--kf-red);" onclick="kfClBuyListing('${listing.id}')">Buy Now — ${money(listing.price_cents)}</button>
                 ${currentUser && currentTokenBalance >= listing.price_cents * 10
                   ? `<button class="submit ready" id="kf-cl-buy-tokens-btn" style="background:#8a6d1f;margin-top:8px;" onclick="kfClBuyWithTokens('${listing.id}', ${listing.price_cents})">🍩 Pay with White Donuts (${(listing.price_cents * 10).toLocaleString()})</button>`
                   : ''}
